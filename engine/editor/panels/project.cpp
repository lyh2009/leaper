...
void Project::OnAttach()
{
    ...
    m_icon_map[".lua"]   = Leaper::Texture::Create("./resource/icons/lua.png");
    m_icon_map[".png"]   = Leaper::Texture::Create("./resource/icons/image.png");
    m_icon_map[".jpg"]   = Leaper::Texture::Create("./resource/icons/image.png");
    m_icon_map[".blend"] = Leaper::Texture::Create("./resource/icons/blend.png");
    m_icon_map[".obj"]   = Leaper::Texture::Create("./resource/icons/blend.png");
    m_icon_map[".fbx"]   = Leaper::Texture::Create("./resource/icons/blend.png");
    m_icon_map[".json"]  = Leaper::Texture::Create("./resource/icons/json.png");
    m_icon_map[".mtl"]   = Leaper::Texture::Create("./resource/icons/json.png");
    m_icon_map[".mp3"]   = Leaper::Texture::Create("./resource/icons/audio.png");
}
...
void Project::OnUpdate()
{
    ...
    if (ImGui::BeginChild("Folder")) { DrawTree(); }
    ImGui::EndChild();

    ImGui::NextColumn();

    if (ImGui::BeginChild("File")) { DrawContent(icon_size); }
    ImGui::EndChild();
...
void Project::DrawTree()
{
    for (auto& directory : std::filesystem::directory_iterator(m_path))
    {
        if (directory.is_directory())
        {
            ...
            ImGui::SameLine();
            if (ImGui::Button(directory.path().filename().string().c_str(), ImVec2(ImGui::GetContentRegionAvail().x, 0))) { m_path /= directory.path().filename(); }
        }
    }
}
