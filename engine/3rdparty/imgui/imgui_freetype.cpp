...
//  2021/01/26: simplified integration by using '#define IMGUI_ENABLE_FREETYPE'. renamed ImGuiFreeType::XXX flags to ImGuiFreeTypeBuilderFlags_XXX for consistency with other API.
//  removed ImGuiFreeType::BuildFontAtlas(). 2020/06/04: fix for rare case where FT_Get_Char_Index() succeed but FT_Load_Glyph() fails. 2019/02/09: added
//  RasterizerFlags::Monochrome flag to disable font anti-aliasing (combine with ::MonoHinting for best results!) 2019/01/15: added support for imgui allocators + added FreeType
//  only override function SetAllocatorFunctions(). 2019/01/10: re-factored to match big update in STB builder. fixed texture height waste. fixed redundant glyphs when merging.
//  support for glyph padding. 2018/06/08: added support for ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved to main imgui repository (away from
//  http://www.github.com/ocornut/imgui_club) 2018/01/22: fix for addition of ImFontAtlas::TexUvscale member. 2017/10/22: minor inconsequential change to match change in master
//  (removed an unnecessary statement). 2017/09/26: fixes for imgui internal changes. 2017/08/26: cleanup, optimizations, support for ImFontConfig::RasterizerFlags,
//  ImFontConfig::RasterizerMultiply. 2017/08/16: imported from https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated for latest changes in
...
namespace
{
    ...
    struct FontInfo
    {
        ...
        float LineSpacing;  // The baseline-to-baseline distance. Note that it usually is larger than the sum of the ascender and descender taken as absolute values. There is also
                            // no guarantee that no glyphs extend above or below subsequent baselines when using this distance. Think of it as a value the designer of the font
                            // finds appropriate.
...
#define FT_CEIL(X) (((X + 63) & -64) / 64)
    ...
    void FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, unsigned char* multiply_table)
    {
        ...
        case FT_PIXEL_MODE_GRAY:  // Grayscale image, 1 byte per pixel.
        {
            ...
            {
                for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                    for (uint32_t x = 0; x < w; x++) dst[x] = IM_COL32(255, 255, 255, multiply_table[src[x]]);
            }
            break;
        }
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
            ...
            if (atlas->TexPixelsAlpha8 != nullptr)
            {
                ...
                for (int y = 0; y < info.Height; y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
                    for (int x = 0; x < info.Width; x++) blit_dst[x] = (unsigned char)((blit_src[x] >> IM_COL32_A_SHIFT) & 0xFF);
            }
            else
            {
    ...
    // Cleanup
    for (int buf_i = 0; buf_i < buf_bitmap_buffers.Size; buf_i++) IM_FREE(buf_bitmap_buffers[buf_i]);
