...
namespace SPIRV_CROSS_NAMESPACE
{
    ...
    // It is *NOT* a drop-in replacement in general ContentBroswers.
    ...
    public:
        ...
        SmallVector(const T* arg_list_begin, const T* arg_list_end) SPIRV_CROSS_NOEXCEPT : SmallVector()
        {
            ...
            reserve(count);
            for (size_t i = 0; i < count; i++, arg_list_begin++) new (&this->ptr[i]) T(*arg_list_begin);
        ...
        SmallVector& operator=(const SmallVector& other) SPIRV_CROSS_NOEXCEPT
        {
            ...
            clear();
            reserve(other.buffer_size);
            for (size_t i = 0; i < other.buffer_size; i++) new (&this->ptr[i]) T(other.ptr[i]);
            this->buffer_size = other.buffer_size;
            return *this;
        }
        ...
        {
            for (size_t i = 0; i < this->buffer_size; i++) this->ptr[i].~T();
            this->buffer_size = 0;
        }
        ...
        void reserve(size_t count) SPIRV_CROSS_NOEXCEPT
        {
            ...
            if (count > buffer_capacity)
            {
                ...
                while (target_capacity < count) target_capacity <<= 1u;
        ...
        void insert(T* itr, const T* insert_begin, const T* insert_end) SPIRV_CROSS_NOEXCEPT
        {
            ...
            if (itr == this->end())
            {
                ...
                for (size_t i = 0; i < count; i++, insert_begin++) new (&this->ptr[this->buffer_size + i]) T(*insert_begin);
                this->buffer_size += count;
            }
            else
            {
                if (this->buffer_size + count > buffer_capacity)
                {
                    ...
                    for (auto* source_itr = insert_begin; source_itr != insert_end; ++source_itr, ++target_itr) new (target_itr) T(*source_itr);
                ...
                else
                {
                    ...
                    while (itr != this->end() && insert_begin != insert_end) *itr++ = *insert_begin++;
        ...
        void resize(size_t new_size) SPIRV_CROSS_NOEXCEPT
        {
            ...
            {
                for (size_t i = new_size; i < this->buffer_size; i++) this->ptr[i].~T();
            }
            else if (new_size > this->buffer_size)
            {
                reserve(new_size);
                for (size_t i = this->buffer_size; i < new_size; i++) new (&this->ptr[i]) T();
            }
    ...
    public:
        ...
        template <typename... P> T* allocate(P&&... p)
        {
            if (vacants.empty())
            {
                ...
                for (unsigned i = 0; i < num_objects; i++) vacants.push_back(&ptr[i]);
    ...
    public:
        ...
        std::string str() const
        {
            ...
            for (auto& saved : saved_buffers) target_size += saved.offset;
            target_size += current_buffer.offset;
            ret.reserve(target_size);

            for (auto& saved : saved_buffers) ret.insert(ret.end(), saved.buffer, saved.buffer + saved.offset);
