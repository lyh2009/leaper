...
// A bit crude, but allows ContentBroswers which embed SPIRV-Cross statically to
// effectively hide all the symbols from other ContentBroswers.
// There is a case where we have:
// - ContentBroswer A links against SPIRV-Cross statically.
// - ContentBroswer A links against ContentBroswer B statically.
// - ContentBroswer B links against SPIRV-Cross statically (might be a different version).
// This leads to a conflict with extremely bizarre results.
// By overriding the namespace in one of the ContentBroswer builds, we can work around this.
...
namespace SPIRV_CROSS_NAMESPACE
{
    ...
    public:
        ...
        {
            lower |= other.lower;
            for (auto& v : other.higher) higher.insert(v);
        }
        ...
        template <typename Op> void for_each_bit(const Op& op) const
        {
            ...
            for (auto& v : higher) bits.push_back(v);
            std::sort(std::begin(bits), std::end(bits));

            for (auto& v : bits) op(v);
        }
...
#define SPIRV_CROSS_DECLARE_CLONE(T)                               \
    ...
    struct SPIRConstantOp : IVariant
    {
        ...
        SPIRConstantOp(TypeID result_type, spv::Op op, const uint32_t* args, uint32_t length) : opcode(op), basetype(result_type)
        {
            ...
            for (uint32_t i = 0; i < length; i++) arguments.push_back(args[i]);
        }
    ...
    struct SPIRConstant : IVariant
    {
        ...
        inline void make_null(const SPIRType& constant_type_)
        {
            ...
            for (auto& c : m.c) c.vecsize = constant_type_.vecsize;
        }
        ...
        SPIRConstant(TypeID constant_type_, const uint32_t* elements, uint32_t num_elements, bool specialized) : constant_type(constant_type_), specialization(specialized)
        {
            ...
            for (uint32_t i = 0; i < num_elements; i++) subconstants.push_back(elements[i]);
...
// #define SPIRV_CROSS_COPY_CONSTRUCTOR_SANITIZE
    ...
    static inline SPIRType::BaseType to_signed_basetype(uint32_t width)
    {
        switch (width)
        {
        case 8: return SPIRType::SByte;
        case 16: return SPIRType::Short;
        case 32: return SPIRType::Int;
        case 64: return SPIRType::Int64;
        default: SPIRV_CROSS_THROW("Invalid bit width.");
        }
    }
    ...
    static inline SPIRType::BaseType to_unsigned_basetype(uint32_t width)
    {
        switch (width)
        {
        case 8: return SPIRType::UByte;
        case 16: return SPIRType::UShort;
        case 32: return SPIRType::UInt;
        case 64: return SPIRType::UInt64;
        default: SPIRV_CROSS_THROW("Invalid bit width.");
        }
    }
    ...
    static inline bool opcode_is_sign_invariant(spv::Op opcode)
    {
        ...
        case spv::OpBitwiseAnd: return true;
