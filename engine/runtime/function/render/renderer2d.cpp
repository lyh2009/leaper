...
void Leaper::Renderer2D::Init(Leaper::Ref<Leaper::RenderAPI> render_api)
{
    ...
    s_data.line_vertex_buffer->SetLayout({{Leaper::ShaderDataType::Float3, "a_Position"},
                                          {Leaper::ShaderDataType::Float4, "a_Color"},
                                          {Leaper::ShaderDataType::Int, "a_EntityID"}});
...
void Leaper::Renderer2D::BeginScene(Leaper::OrthgraphicCamera camera, glm::mat4 &trans)
{
    s_data.camera_buffer.view_projection = camera.GetViewProjectionMat() * glm::inverse(trans);
    StartBatch();
}
