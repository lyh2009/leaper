...
namespace Leaper
{
    ...
    Mesh Model::ProcessMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        std::vector<Mesh::MeshTexture> diffuse_textures  = LoadTexture(material, aiTextureType_DIFFUSE, TextureType::Diffuse);
        std::vector<Mesh::MeshTexture> specular_textures = LoadTexture(material, aiTextureType_SPECULAR, TextureType::Specular);
    ...
    std::vector<Mesh::MeshTexture> Model::LoadTexture(aiMaterial* mat, aiTextureType type, TextureType type_name)
    {
        std::vector<Mesh::MeshTexture> textures;
        for (int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {  // 如果纹理还没有被加载，则加载它
                ...
                texture.texture = Texture::Create(path, false);
