...
OpenGLShader::OpenGLShader(const std::string& shader_path) : m_file_path(shader_path)
{
    ...
    Leaper::Utils::CreateCacheDirectoryIfNeeded();
...
void OpenGLShader::CreateProgramForAmd()
{
    ...
    std::filesystem::path cacheDirectory = Leaper::Utils::GetCacheDirectory();
...
std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
{
    ...
    while (pos != std::string::npos)
    {
        ...
        LP_ASSERT(Leaper::Utils::ShaderTypeFromString(type), "Invalid shader type specified");

        size_t nextLinePos = source.find_first_not_of("\r\n", eol);  // Start of shader code after shader type declaration line
        LP_ASSERT(nextLinePos != std::string::npos, "Syntax error");
        pos = source.find(typeToken, nextLinePos);  // Start of next shader type declaration line

        shader_sources[Leaper::Utils::ShaderTypeFromString(type)] = (pos == std::string::npos) ? source.substr(nextLinePos) : source.substr(nextLinePos, pos - nextLinePos);
    }
...
void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shader_sources)
{
    ...
    {
        std::filesystem::path shaderFilePath = m_file_path;
        std::filesystem::path cachedPath     = cacheDirectory / (shaderFilePath.filename().string() + Leaper::Utils::GLShaderStageCachedVulkanFileExtension(stage));
        ...
        else
        {
            ...
            shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Leaper::Utils::GLShaderStageToShaderC(stage), m_file_path.c_str(), options);
...
void OpenGLShader::CompileOrGetOpenGLBinaries()
{
    ...
    for (auto&& [stage, spirv] : m_vkSPIRV)
    {
        ...
        std::filesystem::path shaderFilePath = m_file_path;
        std::filesystem::path cachedPath     = cacheDirectory / (shaderFilePath.filename().string() + Leaper::Utils::GLShaderStageCachedOpenGLFileExtension(stage));
        ...
        else
        {
            ...
            shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Leaper::Utils::GLShaderStageToShaderC(stage), m_file_path.c_str());
            if (module.GetCompilationStatus() != shaderc_compilation_status_success)
            {
                LP_CORE_LOG_ERROR(module.GetErrorMessage());
            }
...
void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shader_data)
{
    ...
    LP_CORE_LOG_INFO("OpenGLShader::Reflect - {0} {1}", Leaper::Utils::GLShaderStageToString(stage), m_file_path);
